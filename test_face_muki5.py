...
# 5秒おきの顔の移動量に変える
def change_60_second(all_change):
    change_60_second = []
    for z in all_change:
        x_all = 0
        y_all = 0
        ...
        for i in z:
            if None in i:
                print(z)
                change_60_second.append([None, None])
                break
            x_all += abs(i[0])
            y_all += abs(i[1])
        change_60_second.append([x_all, y_all])
    return change_60_second
...
def cv_main(video_path, right_t_provisional, left_t_provisional):
    ...
    while True:
        ...
        if rect_frag == False:
            section_5_angle_list.append([angle_threshold_yaw, angle_threshold_pitch, angle_threshold_roll])
            section_5_change_list.append([None, None])
        ...
        if section_60_frame_cnt == 1800:
            ...
            all_60_blink_list.append(blink_60_cnt)
            section_60_change_list = []
    ...
    return all_5_blink_list, all_5_change_list, all_5_angle_list,all_60_blink_list, all_60_change_list, all_60_angle_list
...
if __name__ == '__main__':
    ...
    # 動画の処理をするmain関数
    all_5_blink_list, all_5_change_list, all_5_angle_list,all_60_blink_list, all_60_change_list, all_60_angle_list = cv_main(file_path, right_threshold, left_threshold)

    # print(all_change_list)

    # 5秒おきの顔の変化量を全部足した
    change_5_sec = change_5_second(all_5_change_list)
    change_60_sec = change_5_second(all_60_change_list)
